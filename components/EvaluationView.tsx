...
export const EvaluationView: React.FC<EvaluationViewProps> = ({ evaluation }) => {
  ...
  return (
    <div className="space-y-8 animate-fade-in">
      ...
      {/* Deep Dive Analysis (Follow Up) */}
      {evaluation?.followUp && evaluation.followUp.length > 0 && (
        <div className="bg-gradient-to-br from-indigo-50 to-white rounded-xl shadow-sm border border-indigo-100 p-6">
          <div className="flex items-center gap-3 mb-4">
            ...
            <h3 className="text-lg font-bold text-slate-800">深度追问分析 (Deep Dive Analysis)</h3>
          </div>
          <div className="space-y-4">
            {evaluation.followUp.map((text, idx) => (
              <div key={idx} className="bg-white p-4 rounded-lg border border-indigo-50 shadow-sm">
                ...
                <p className="text-slate-700 whitespace-pre-wrap leading-relaxed">{text}</p>
              </div>
            ))}
          </div>
        </div>
